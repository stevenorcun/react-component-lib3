...
export type applicative_config =
  | "config"
  | "external-apps"
  | "custom-actions"
  | "keybind"
...
export default class ApplicativeApi extends BaseApi {
  ...
  public async getGlobalConfig(): Promise<AppSettings> {
    try {
      const response = await super
        .init()
        .get<AppSettings>(`/${this.uri_segment}`);
  ...
  public async createConfig<T>(
    ...
    try {
      const response = await super
        .init()
        .post<DbConfig<T>>(`/${this.uri_segment}/${type}`, data);
      return response.data;
    } catch (err) {
      throw this.handleError(err);
    }
  ...
  public async updateConfig<T>(
    ...
    try {
      const response = await super
        .init()
        .put<DbConfig<T>>(`/${this.uri_segment}/${type}/${data.id}`, data);
      return response.data;
    } catch (err) {
      throw this.handleError(err);
    }
  ...
  public async deleteConfig<T>(
    ...
    try {
      const response = await super
        .init()
        .delete<void>(`/${this.uri_segment}/${type}/${data.id}`);
      return response.data;
    } catch (err) {
      throw this.handleError(err);
    }
